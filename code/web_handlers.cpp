...
// JSON escape helper for config values
...
// 处理配置页面请求 — 生成 JSON 注入到前端
void handleRoot() {
  if (!checkAuth()) return;

  String html = String(htmlPage);

  // 构建配置 JSON，Vue 前端通过 window.__CFG__ 读取
  ...
  String j = "{";
  j += "\"ip\":" + jsonStr(WiFi.localIP().toString()) + ",";
  j += "\"wifi_ssid\":" + jsonStr(String(WiFi.SSID())) + ",";
  j += "\"free_heap\":" + jsonStr(String(ESP.getFreeHeap() / 1024) + " KB") + ",";
  j += "\"uptime\":" + jsonStr(String(uptimeBuf)) + ",";
  j += "\"web_user\":" + jsonStr(config.webUser) + ",";
  j += "\"web_pass\":" + jsonStr(config.webPass) + ",";
  j += "\"smtp_server\":" + jsonStr(config.smtpServer) + ",";
  j += "\"smtp_port\":" + jsonStr(String(config.smtpPort)) + ",";
  j += "\"smtp_user\":" + jsonStr(config.smtpUser) + ",";
  j += "\"smtp_pass\":" + jsonStr(config.smtpPass) + ",";
  j += "\"smtp_send_to\":" + jsonStr(config.smtpSendTo) + ",";
  j += "\"admin_phone\":" + jsonStr(config.adminPhone) + ",";
  j += "\"number_black_list\":" + jsonStr(config.numberBlackList) + ",";
  j += "\"smtp_check\":" + jsonStr(emailOk ? "已配置" : "未配置") + ",";
  j += "\"modem_check\":" + jsonStr(modemReady ? "已就绪" : "未就绪") + ",";
  j += "\"push_count\":" + jsonStr(String(pushCount));

  // 推送通道配置
  for (int i = 0; i < MAX_PUSH_CHANNELS; i++) {
    String pfx = "push" + String(i);
    j += ",\"" + pfx + "_enabled\":" + String(config.pushChannels[i].enabled ? "1" : "0");
    j += ",\"" + pfx + "_type\":" + jsonStr(String((int)config.pushChannels[i].type));
    j += ",\"" + pfx + "_name\":" + jsonStr(config.pushChannels[i].name);
    j += ",\"" + pfx + "_url\":" + jsonStr(config.pushChannels[i].url);
    j += ",\"" + pfx + "_key1\":" + jsonStr(config.pushChannels[i].key1);
    j += ",\"" + pfx + "_key2\":" + jsonStr(config.pushChannels[i].key2);
    j += ",\"" + pfx + "_body\":" + jsonStr(config.pushChannels[i].customBody);
  }
  j += "}";

  html.replace("/*%CFG_JSON%*/{}", j);
